...
Next up is to get it containerized locally. When we build a container, Docker will use a Dockerfile to specify how we want the container image to be created. The container image is "a lightweight, standalone, executable package of software that includes everything needed to run an application: code, runtime, system tools, system libraries and settings." If we had multiple containers in our service, we could create a `compose.yaml` file to help specify containers, volumes, ports, etc., but that's not necessary yet. 
